...
public class CastlingMoveValidatorImpl implements CastlingMoveValidator {
    public Move getCastlingMoveIfPossible(DomainBoard board, PositionDto actualPosition, MoveDto moveToMake, PieceColor turn, List<Move> moves, PositionValidator positionHandler, CheckMovementValidator checkMovementValidator) {
    /* - castling is only possible if neither the king nor the rook has moved
       ...
       -the square the king goes to and any intervening squares may not be under attack
        ...
        Piece actualPiece = board.getPiece(actualPosition.getX(),actualPosition.getY());
        ...
        if (moveToMake.getDestinationX() == xPositionByTurn && moveToMake.getDestinationY() == 6 || moveToMake.getDestinationX() == xPositionByTurn && moveToMake.getDestinationY() == 2) {
            if (positionHandler.hasPieceMoved(new PositionDto(xPositionByTurn, originYPositionKing), moves)) { // check that the king (the origin piece should be a king for being able to castle) hasnt moved
                return null; // error
            }
            ...
            if (positionHandler.hasPieceMoved(new PositionDto(xPositionByTurn, rookOriginY), moves)) {
                return null;
            }
            MoveDto castlingMove = new MoveDto(xPositionByTurn, originYPositionKing, xPositionByTurn, rookOriginY);
            if ((positionHandler.arePiecesInBetween(board, castlingMove))) { // if there are pieces in the middle we return
                return null;
            }
            for (PositionDto positionDto : positionHandler.getPositionsInBetween(board, new NotFilterStrategy(), xPositionByTurn, originYPositionKing, xPositionByTurn, rookOriginY)) {
                if (positionHandler.isSquareBeingAttacked(board, positionDto, turn)) { // if a position is being attacked, castling cannot be made
                    return null; // error
                }
